...
```bash
sudo ./rockpi-backup.sh -o <backup-file>.img # 进行重命名，不改变输出位置

sudo ./rockpi-backup.sh -o /mnt/sda1/ # 不进行重命名，改变输出位置

sudo ./rockpi-backup.sh -o /mnt/sda1/<backup-file>.img # 进行重命名并改变输出位置
```
...
```bash
sudo ./rockpi-backup.sh -m $ROOT_MOUNT_POINT -o <backup-file>.img # 进行重命名，不改变输出位置

sudo ./rockpi-backup.sh -m $ROOT_MOUNT_POINT -o /mnt/sda1/ # 不进行重命名，改变输出位置

sudo ./rockpi-backup.sh -m $ROOT_MOUNT_POINT -o /mnt/sda1/<backup-file>.img # 进行重命名并改变输出位置
```
...
```bash
dd if=/dev/sdx of=<backup-file>.img bs=4M status=progress # /dev/sdx 替换为真实的设备路径 <backup-file>.img替换为输出的文件名
```
