...
```bash
# mali
sudo dmesg | grep fb000000.gpu-mali

# Panthor
sudo dmesg | grep fb000000.gpu
```

### 验证 xorg-xserver 版本

运行以下命令，查看当前 `[installed,automatic]` 对应的 xorg-xserver 版本，使用 mali 驱动时，为 `rk3588*-bookworm`，使用 Panthor 驱动时，版本为 `stable*`。
