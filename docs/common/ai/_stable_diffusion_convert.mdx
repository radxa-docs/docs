...
本次演示使用的 **Stable Diffusion LCM Dreamshaper V7**，是一种结合 Latent Consistency Model 加速技术的轻量化变体，能够在极低推理步数下保持高质量的图像生成效果，仅需 **1–4 步** 即可生成清晰的图像。
本文将完整演示如何借助 RKNN 工具链，把该模型部署到 Rockchip 系列芯片的 NPU 上，并实现高效、低延迟的端侧图像生成。


:::tip
此文档以RK3588及Dreamshaper V7为例，
演示如何在RK芯片NPU上部署文生图模型，
需要预先在PC端配置好RKNN相关环境，
...
- **将转换后的RKNN模型及可执行文件拷贝到板端**
   ...
   <NewCodeBlock tip="Radxa SBC" type="device">
   ```bash
   pip3 install diffuser，pillow，"numpy<2.0"，torch，transformers，rknn-toolkit-lite2
   ```
   </NewCodeBlock>
