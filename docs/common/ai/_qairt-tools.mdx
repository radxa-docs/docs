此文档介绍 QAIRT SDK 在做模型移植和部署时主要使用的 CLI 工具的使用方法

## CLI 工具表格

| CLI 名称                                                      | CLI 用途                       |
| ------------------------------------------------------------- | ------------------------------ |
| [qairt-converter](#qairt-converter)                           | 转换 DLC 模型                  |
| [qairt-quantizer](#qairt-quantizer)                           | 量化 DLC 模型                  |
| [qnn-context-binary-generator](#qnn-context-binary-generator) | 转换 Context Binary 模型       |
| [qnn-net-run](#qnn-net-run)                                   | 推理 DLC / Context Binary 模型 |
| [qnn-context-binary-utility](#qnn-context-binary-utility)     | 检查 Context Binary 模型       |

## CLI Tools

### qairt-converter

`qairt-converter` 工具可将 Onnx/TensorFlow/TFLite/PyTorch 框架中的模型转换为 QNN 图形格式的 DLC 文件，
`qairt-converter` 会根据源模型扩展自动检测框架。当前的 ONNX 转换最高支持 ONNX Opset 21。
...
`qairt-quantizer` 工具可将非量化 DLC 模型转换为量化 DLC 模型。
...
`qnn-net-run` 工具用于使用特定后端推理经过 QAIRT 转换的格式模型
...
`qnn-context-binary-utility` 工具会验证 Context-Binary 模型文件的元数据并将其序列化为 JSON 文件。此 JSON 文件可用于检查 Context-Binary 模型，从而辅助调试。
...
关于详细和完整的 QAIRT Tools 的文档请参考

**本地文档**

- [**QAIRT SDK 本地文档**](qairt-install#sdk-完整文档) 下的 `docs/QNN/general/tools.html`
