...
这里以上一章节的 [MobileNetV2_ImaegNet](./cubie-acuity-usage#量化模型) 的 uint8 为例子，此模型直接使用 ACUITY 进行 uint8 量化后精度损失，推理得到的结果与浮点模型存在明显精度损失。
...
接下来可以继续 NPU 的部署工作进行 [模型编译并导出](./cubie-acuity-usage#模型编译并导出), **因为混合量化后模型结构发生改变，在进行模型转换时请使用混合量化生成的 ACUITY 模型结构 `quantize.json` 文件名作为 `--model` 参数**。
