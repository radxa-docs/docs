Stable Diffusion 是一个可以根据文本生成相应场景照片的生成式大模型，目前使用 [StableDiffusion 1.5](https://huggingface.co/runwayml/stable-diffusion-v1-5)
开源模型通过 sophon SDK 移植到 SG2300X 芯片系列产品上进行本地 TPU 硬件加速推理，结合 LCM Lora 加速模块与风格 LoRa 实现快速推理生成特色风格图片，并使用 Gradio 实现用户交互
...
- 下载 Stable Diffusion v1.5 风格 Lora models 压缩包
  ...
  用户也可通过 model_export.py 与 [TPU-MLIR](../../model-compile) 编译任何 stable_diffusion v1.5 checkpoints, 具体请参考 [model_export/README](https://github.com/zifeng-radxa/SD-lcm-tpu/blob/radxa_v0.2.0/model_export/README.md)
