...
:::tip
如还没安装 TPU-MLIR 的运行环境，请参考[上一章节](tpu_mlir_env)
:::

### 模型转换

本案例以 [yolov5s.onnx](https://github.com/ultralytics/yolov5/releases/download/v6.0/yolov5s.onnx) 为例, 介绍如何编译并迁移一个 onnx 模型至 SG2300X TPU 平台运行，本章节模型编译都在[上一章节](tpu_mlir_env)安装的 TPU-MLIR docker 中运行。
