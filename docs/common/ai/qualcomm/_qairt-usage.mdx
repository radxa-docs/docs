...
生成目标量化模型保存在 `resnet50_quantized.dlc`。关于更多 `qairt-quantizer` 使用方法请参考 [**qairt-quantizer**](qairt-tools#qairt-quantizer)。
...
- config_backend.json
  ...
  <TabItem value="Q6490" default={props.tag === "q6490"} attributes={{className: props.tag !== "q6490" && "tab_none"}}>
          ...
          请根据 SoC NPU 架构选择相应的 dsp_arch 和 soc_id， 这里以 Q6490 SoC 为例子
          ...
          请根据 SoC NPU 架构选择相应的 dsp_arch 和 soc_id， 这里以 Q8280 SoC 为例子
          ...
          请根据 SoC NPU 架构选择相应的 dsp_arch 和 soc_id， 这里以 Q9075 SoC 为例子
...
生成 Context-Binary 保存在 `output/resnet50_quantized.bin`。关于更多 `qnn-context-binary-generator` 使用方法请参考 [**qnn-context-binary-generator**](qairt-tools#qnn-context-binary-generator)。
...
- 拷贝 NPU 架构专用动态库文件到板端
  ...
  请根据 SoC NPU 架构选择相应的 hexagon 文件夹, 这里以 Q6490 为例子
...
- 执行模型推理
  ...
  结果保存在 `output_bin` 中。关于更多 `qnn-net-run` 使用方法请参考 [**qnn-net-run**](qairt-tools#qnn-net-run)。
