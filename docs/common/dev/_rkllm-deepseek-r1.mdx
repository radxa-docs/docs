...
本文档将详细演示如何利用 RKLLM 工具链，将蒸馏模型 DeepSeek-R1-Distill-Qwen-1.5B 部署至 RK3588 主板，并调用其内置的 NPU 实现高效的硬件加速推理。
...
虚拟环境配置参考[创建虚拟环境](./rkllm-install#创建虚拟环境)。
...
交叉编译工具链配置参考[编译工具](./rkllm-install#编译工具)。
