...
- 运行 llm_demo，输入 `exit` 退出
  ...
  :::tip
  如果这里报错 `failed to open rknpu module` 或 `failed to open rknn device`，通常说明板端缺少 RKNPU2 用户态包，或驱动版本未满足 RKLLM 要求。请先回到 [RKLLM 安装](./rkllm_install#板端驱动配置) 中的板端驱动配置章节，确认 `rknpu2-rk3588` 已安装，并检查 `/sys/kernel/debug/rknpu/version`。
  :::

  | 参数              | 必要性 | 描述                   | 选项                             |
  | ----------------- | ------ | ---------------------- | -------------------------------- |
  | `path`            | 必要   | RKLLM 模型文件路径（.rkllm）。 | N                                |
...
{/* 用户可以在主板开启 gradio 服务端后在同网络环境其他设备上通过 Gradio API 调用 LLM gradio server */}
...
{/* 用户可以在主板开启 flask 服务端后在同网络环境其他设备上通过 flask API 调用 flask server, 户在进行自定义功能的开发的过程中，只需参考该 API 访问示例使用对应的收发结 */}
{/* 构体进行数据的包装、解析即可 */}
