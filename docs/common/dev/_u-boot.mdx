...
以 Radxa ZERO 3 为例，烧录官方镜像，等待系统正常运行后，在开发板上执行 `sudo apt list --installed | grep u-boot` 获取 U-Boot 包名为 `u-boot-latest`。  
然后将 U-Boot 包名后缀 `latest` 填入 bsp 命令的 `profile` 参数例如：`./bsp u-boot latest radxa-zero3`，即可构建 U-Boot。  
