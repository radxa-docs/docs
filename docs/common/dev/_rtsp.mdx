...
nohup ./mediamtx &
pushd ffmpeg-rockchip/
./ffmpeg -f v4l2 -i /dev/video-camera0 -vf "fps=30" -s 3840x2160 -c:v h264_rkmpp -rc_mode AVBR -b:v 20M -minrate 10M -maxrate 40M -profile:v main -level 5.1 -f rtsp rtsp://0.0.0.0:8554/stream

# 如果仅想将摄像头录制成文件，请使用以下命令
# ./ffmpeg -f v4l2 -i /dev/video-camera0 -vf "fps=30" -s 3840x2160 -c:v h264_rkmpp -rc_mode AVBR -b:v 20M -minrate 10M -maxrate 40M -profile:v main -level 5.1 /tmp/tmp.mp4
popd
```
...
:::tip
因为 Gstreamer 在 6.1 内核的 Radxa OS 编解码效率很差，此方法不太适用于 6.1 内核的 Radxa OS。
...
```bash
pushd ffmpeg-rockchip/
./ffplay -vcodec h264_rkmpp rtsp://192.168.10.9:8554/stream
popd
```
