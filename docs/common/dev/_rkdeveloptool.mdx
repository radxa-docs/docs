...
```bash
sudo apt-get update
sudo apt-get install -y python3-pyqt6 nuitka git
...
python -m nuitka --standalone --onefile --follow-imports \
       ...
       --output-dir=dist \
       rkdevtoolgui.py
sudo cp dist/rkdeveloptool-gui /usr/local/sbin/
```
...
```bash
brew install python3-pyqt6 nuitka git
git clone https://github.com/gahingwoo/RKDevelopTool-GUI.git
cd RKDevelopTool-GUI
python -m nuitka --standalone --onefile --follow-imports \
       --enable-plugin=pyqt6 \
       ...
       --output-dir=dist \
       rkdevtoolgui.py
cp dist/rkdeveloptool-gui /opt/homebrew/bin/
```
