...
1. 解压镜像文件 (镜像文件详见上文 "文件下载" 部分， 此处 以 rock5_debian_bullseye_desktop\_release_b39.img.xz 为例)

   xz -d rock5_debian_bullseye_desktop\_release_b39.img.xz (如果是gz格式，请使用gunzip -d xxx.gz命令)

2. 烧录 Loader (实际文件见上文 “文件下载” 部分，这里以 rk3528_spl_loader_v1.xxx.bin 为例)

   sudo rkdeveloptool db rk3528_spl_loader_v1.xxx.bin

3. 烧录镜像 (以下镜像名只是示例，实际镜像文件是步骤1解压后的文件)

   sudo rkdeveloptool wl 0 rock5_debian_bullseye_desktop\_release_b39.img
