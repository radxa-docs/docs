...
<Image
  src="/img/common/other-system/android/#sbc_model#-adb-debug.webp"
...
1. 进入主板的 Android 系统的 `Settings` → `About tablet` 界面。
2. 点击 `Build number` 七次，进入开发者模式。
3. 进入主板的 Android 系统的 `Settings` → `System` → `Developer options` 界面。
