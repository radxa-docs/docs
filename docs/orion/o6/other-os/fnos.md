...
本文介绍如何在 **Radxa Orion O6**（基于 CIX 此芯 P1 SoC）上安装 **飞牛 fnOS（FnNAS）** ARM 公开测试版系统，并指向飞牛官方的安装教程与帮助中心。

:::warning 适用范围与当前状态

- 本文档仅适用于 **Radxa Orion O6**（瑞莎星睿 O6，SoC 为 CIX 此芯 P1）
- 飞牛 fnOS 在 Orion O6 上的支持属于 **社区验证**，不是 Radxa 官方维护的发行版
...
- 已将设备 BIOS 更新到较新版本：请参考 [BIOS 更新](../low-level-dev/bios.md)
- 已准备一个容量不小于 16 GB 的 U 盘（推荐 USB 3.0）
- 已准备一块用于 fnOS 系统盘的 SSD，建议容量 ≥ 64 GB
...
找到镜像后，请在页面中按提示自行下载当前最新版本。具体版本号和构建号会随公测迭代变化，请以飞牛 ARM 下载页为准。
...
1. 将写好的 U 盘插到 Orion O6，进入 BIOS 启动菜单选择从 U 盘启动
2. 在安装菜单选择 `Graphical Install`（图形化安装）
3. 选择 fnOS 系统盘所在的 SSD，按提示完成分区与 Swap 设置（系统分区建议 ≥ 64 GB）
4. 确认格式化并开始安装
