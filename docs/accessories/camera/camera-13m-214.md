...
:::note 关于 Cubie A7Z 的兼容性
根据用户反馈，Radxa 13M 214 摄像头（IMX214 传感器）在 Cubie A7Z（Allwinner A733）平台上可能无法正常工作。在 Cubie A7Z 上使用本摄像头时，可能会出现以下问题：

- 摄像头不被系统检测
- 没有创建 `/dev/video*` 设备节点
- IMX214 驱动程序未加载

如果您在 Cubie A7Z 上遇到摄像头检测问题，请检查：
1. 当前内核版本是否包含 IMX214 驱动支持
2. 设备树配置是否正确
3. 可能需要额外的内核补丁或配置

建议在 Cubie A7Z 上使用前，先确认您的系统镜像已包含完整的摄像头驱动支持。
:::
