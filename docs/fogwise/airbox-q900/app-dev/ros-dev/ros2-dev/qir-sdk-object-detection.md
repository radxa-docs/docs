...
Q900 当前提供的 `qrb_ros_nn_inference` 1.1.1 和 `qrb_ros_cv_tensor_common_process` 0.0.0 无法直接运行当前 QAI Hub 导出的 version 3 context binary。需要将接口、推理、张量处理和示例源码放在同一个工作区构建，确保所有组件使用相同的 `TensorList` ABI。
...
通过 QAI Hub 生成 QCS9075 context binary。
...
该命令会下载 YOLOv8-N 权重，在 QAI Hub 上编译模型并下载 `yolov8_det.bin`。
...
`sample_object_detection` 没有声明对图片预处理包的源码依赖，需要显式构建：
...
实机捕获的 `/yolo_detect_overlay` 如下：

![YOLOv8 目标检测结果](/img/fogwise/airbox-q900/airbox-q900-qir-sdk-object-detection-result.webp)
...
`/yolo_detect_overlay` 应为 `640 x 640` 的 `bgr8` 图片。需要图形化查看时，在与 Q900 使用相同 `ROS_DOMAIN_ID` 的 ROS 2 Jazzy 桌面环境中启动 `rqt`，依次选择 **Plugins > Visualization > Image View**，再选择 `/yolo_detect_overlay`。
...
- 输入图片会缩放为 `640 x 640`，检测框坐标对应缩放后的图片。
