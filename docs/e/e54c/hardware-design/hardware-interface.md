...
- **千兆网络接口**：4 个千兆以太网端口由板载 RTL8367RB 交换芯片（U20）扩展，交换芯片通过一路 RGMII 上行连接到 SoC 的 GMAC1
...
- **端口间硬件转发**：4 个网口之间的二层转发由交换芯片硬件完成，不占用 CPU；但需要经过 CPU 的流量共用同一条 1Gbps RGMII 上行
- **高速存储访问**：M.2 PCIe 接口通过高速总线扩展存储性能
- **多接口并行访问**：不同接口使用独立数据通道，提高了整体系统吞吐量

:::note 四个网口共用一条上行链路

E54C 的 4 个 RJ45 网口不是从 SoC 直接引出的 4 个独立 MAC，而是由板载交换芯片（RTL8367RB，U20）扩展：SoC 侧只占用一路 GMAC1（RGMII，1Gbps），再经交换芯片分到 4 个网口。

- 网口与网口之间的二层转发由交换芯片硬件完成，端口间可以线速互转，不占用 CPU；
- 需要经过 CPU 的流量（路由 / NAT / 防火墙 / 多 WAN 负载均衡 / 送 CPU 的 VLAN 流量等）共用这条 1Gbps 上行，因此四个网口同时跑路由的总吞吐上限约 1Gbps，而不是 4Gbps；
- WAN / LAN 划分依靠软件（VLAN / 交换芯片配置，如 iStoreOS、OpenWrt），而不是 4 张独立网卡。
...
E54C 的 4 个千兆网络接口由板载 RTL8367RB 交换芯片扩展，交换芯片通过 RGMII 连接到瑞芯微 RK3582 SoC 集成的千兆网络控制器（GMAC1）。相关特性包括：
