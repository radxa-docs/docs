...
转换完成后，在 `mobilenet_v3_large/` 目录下生成：
...
```bash
scp mobilenet_v3_large_mtk_int8.tflite <user>@<device>:/path/to/nio12l-model-zoo/models/mobilenet_v3_large/int8/
scp mobilenet_v3_large_mtk_fp32.tflite <user>@<device>:/path/to/nio12l-model-zoo/models/mobilenet_v3_large/fp32/
```
...
结果保存在 `outputs/mobilenet_v3_large/classifications/` 目录下。
