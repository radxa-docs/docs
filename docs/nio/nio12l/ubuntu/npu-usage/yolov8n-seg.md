---
sidebar_position: 22
description: NIO12L 平台 YOLOv8n-Seg 模型的 Host 端转换与 Device 端部署完整流程
---
...
转换完成后，在 `examples/yolov8n-seg/model/` 目录下生成：
...
结果保存在 `outputs/yolov8n-seg/` 目录下（`vis/` 为可视化图片，`detections/` 为 JSON）。
