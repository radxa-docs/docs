...
function saveJson(data) {
  const outPath = path.resolve(getRepoDir(), 'static/json/commit_update_data.json');
...
async function httpGetWithRetry(url, config = {}, retries = 3, backoffMs = 1000) {
  let lastErr;
...
async function listCommits(days = 30, perPage = 100, maxPages = 3) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    if (headers && headers['x-ratelimit-remaining'] !== undefined) {
      console.log(`GitHub API 剩余配额: ${headers['x-ratelimit-remaining']}/${headers['x-ratelimit-limit']}`);
    }
...
async function getMarkdownCommits(days = 30) {
  ...
  for (const item of list) {
    try {
      ...
      const authorName = (detail.commit && detail.commit.author && detail.commit.author.name) || (item.commit && item.commit.author && item.commit.author.name) || (item.author && item.author.login) || 'unknown';
      const committedAt = (detail.commit && detail.commit.author && detail.commit.author.date) || (item.commit && item.commit.author && item.commit.author.date) || item.committer && item.committer.date || '';
      const title = (detail.commit && detail.commit.message ? detail.commit.message.split('\n')[0] : '') || (item.commit && item.commit.message ? item.commit.message.split('\n')[0] : '');
      const message = (detail.commit && detail.commit.message) || (item.commit && item.commit.message) || '';
