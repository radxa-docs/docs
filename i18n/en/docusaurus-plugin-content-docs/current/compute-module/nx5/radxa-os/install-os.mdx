...
2. Power up the NX5 IO via the DC adapter.
...
&emsp;[upgrade_tool](./low-level-dev/upgrade-tool.md) (Can be installed to multiple devices simultaneously)
...
1. Power on the NX5 IO via the DC adapter.
2. The NX5 IO will boot up with the green power LED on, and the blue LED will start blinking after entering the system.
