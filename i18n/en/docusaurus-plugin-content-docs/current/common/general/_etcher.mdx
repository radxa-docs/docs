...
Please download [balenaEtcher-1.19.16.Setup.exe](https://github.com/balena-io/etcher/releases/download/v1.19.16/balenaEtcher-1.19.16.Setup.exe) After the download is complete, there is no need to perform any additional actions. After downloading, you can double click it to open it without any additional operation.
...
After successfully burning the microSD card according to the above steps, insert the microSD card into the MicroSD slot of {props.product}, power on the system with the {props.power_supply} power adapter, and then the system will start booting and HDMI will display the desktop.
