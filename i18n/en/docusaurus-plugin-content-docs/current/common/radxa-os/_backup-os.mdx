...
<Tabs groupId="method" queryString>
  <TabItem value="online" label="Online Backup" default>
    Run the following command on your motherboard:
  </TabItem>
...
<Tabs groupId="method" queryString>
  <TabItem value="online" label="Online Backup" default>
Run the following command on your motherboard and follow the prompts.
...
# sudo ./rockpi-backup.sh -m $ROOT_MOUNT_POINT -o <backup-file>.img
```
...
--------------------
Checking disk...
The backup file will be saved at /home/radxa/<backup-file>.img
...
Backup done, the file is /home/radxa/<backup-file>.img
```
...
```bash
sudo ./rockpi-backup.sh -o <backup-file>.img # Rename without changing the output location

sudo ./rockpi-backup.sh -o /mnt/sda1/ # Do not rename, change the output location

sudo ./rockpi-backup.sh -o /mnt/sda1/<backup-file>.img # Rename and change the output location
```
...
```bash
sudo ./rockpi-backup.sh -m $ROOT_MOUNT_POINT -o <backup-file>.img # Rename without changing the output location

sudo ./rockpi-backup.sh -m $ROOT_MOUNT_POINT -o /mnt/sda1/ # Do not rename, change the output location

sudo ./rockpi-backup.sh -m $ROOT_MOUNT_POINT -o /mnt/sda1/<backup-file>.img # Rename and change the output location
```
...
```bash
dd if=/dev/sdx of=<backup-file>.img bs=4M status=progress # Replace /dev/sdx with the actual device path, and replace <backup-file>.img with the output file name
```
