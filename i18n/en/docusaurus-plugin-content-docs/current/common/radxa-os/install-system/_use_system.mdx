...
[Display-less mode](../system-config/screenless) refers to using the system without a monitor. Users are recommended to access and control the system through [serial console login](../system-config/uart-login) or [SSH login](../system-config/ssh-login).
