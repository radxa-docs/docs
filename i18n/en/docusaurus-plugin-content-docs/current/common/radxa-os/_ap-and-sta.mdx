...
In the command, select myAcc as the hotspot by using the ifname parameter; at this point, myAcc will be set to AP mode. You can set the hotspot's name and password by specifying `ssid <ssid>` and `password <password>`.
