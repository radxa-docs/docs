...
- 1.8V adapter
- Connection accessory (recommended):
  - Orion O6: SOP8 208mil chip socket
  - Orion O6N: programming pins
- Fallback connection accessory: SOP8 clip (not recommended as the first choice, see note below)
- Linux PC
...
### Assemble the Programming Clip (Fallback Option)

:::caution The clip is only a fallback option

- Clamping the chip with the clip risks damaging the chip
- Repeated clamping can wear out the clip, so it may no longer grip the chip firmly on the board

We recommend using the SOP8 208mil chip socket (Orion O6) or programming pins (Orion O6N) to connect the SPI programmer instead.
