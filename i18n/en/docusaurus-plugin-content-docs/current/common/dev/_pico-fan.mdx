##### FAN
    ...
    The purpose of this paragraph is to provide the user with an example of the host CPU communicating with the MCU RP2040 by obtaining the temperature of the host CPU and making the fan spin up when the specified temperature is reached.

- Connetion

    Connect the fan's pwm pin to PIN_3, the fan VCC to {props.product_name}'s vCC, and the fan GND to {props.product_name}'s GND.

- Install the required Python libraries On the Host
...
- Create a new Temperature.py file on the host with the following contents:
...
- Add a directory for pwm_fan inside pico-examples/pwm/CMakeLists.txt
...
- Create a new CMakeLists.txt in the pico-examples/pwm/pwm_fan/ directory with the following contents:
...
- Create a new pwm_fan.c in the pico-examples/pwm/pwm_fan/ directory with the following contents:
...
- Run Temperature.py on the board

    ```bash
    sudo python3 Temperature.py
    ```

    If the program runs successfully, the program gets the current temperature of the host computer, sends the temperature to the serial port /dev/ttyS0, and formats the output temperature such as “CPU Temperature: 42.0°C”

- Run the minicom on the board

    ```bash
    sudo minicom -D /dev/ttyS0 -b 115200
    ```

    Check if the RP2040 has received the temperature from the host, if it has, the minicom will output the corresponding temperature, e.g. “Received temperature: 42.00°C”.
