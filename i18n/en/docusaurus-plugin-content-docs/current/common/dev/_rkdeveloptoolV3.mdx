...
1. Extract the image file (See **File Download** above for details of the image file, here rock5_debian_bullseye-test_desktop_release_b39.img.xz is used as an example)

```bash
xz -d rock5_debian_bullseye-test_desktop_release_b39.img.xz
```

2. Flash Loader(See **File Download** above for details of the loader file，here rk3528_spl_loader_v1.xxx.bin is just an example)

```bash
sudo rkdeveloptool db rk3528_spl_loader_v1.xxx.bin
```

3. Flash System Image (Here rock5_debian_bullseye-test_desktop_release_b39.img is an example, use the file got from step1 as a replacement)

```bash
sudo rkdeveloptool wl 0 ock5_debian_bullseye-test_desktop_release_b39.img
```
