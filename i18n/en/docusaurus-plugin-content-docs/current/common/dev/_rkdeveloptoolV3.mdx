...
```bash
xz -d rock_debian_bullseye-_desktop_release_b1.img.xz (if the image file is gz format，then use `gunzip -d <image>.gz` command)
```

2. Flash Loader(See **File Download** above for details of the loader file，here `<loader-file>.bin` is just an example)
