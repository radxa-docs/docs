...
Use a 15-Pin 1.0 mm pitch to {props.pins} {props.pitch} {props.orientation} FPC cable.
...
1. Use the arrow keys to find the [ ] {props?.enable_camera ?? 'Enable Radxa Camera 8M 219'} option.
2. Press `Space` so it changes to [*] {props?.enable_camera ?? 'Enable Radxa Camera 8M 219'}, indicating that the feature is enabled.
