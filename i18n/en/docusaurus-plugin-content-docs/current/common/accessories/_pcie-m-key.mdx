...
2. Reading test
   ...
   <pre style={{ whiteSpace: "nowrap" }}>
     radxa@{props.model}:~$ fio -rw=read -bs=1m -runtime=60 -iodepth 1
...
<img src={props.write_test_img} alt="read test" style={{ width: "80%" }} />

3. Writing test

   <pre style={{ whiteSpace: "nowrap" }}>
     radxa@{props.model}:~$ fio -rw=write -bs=1m -runtime=60 -iodepth 1
     -numjobs=8 -size=1G -filename /dev/nvme0n1 -ioengine libaio -direct=1
...
<img src={props.read_test_img} alt="write test" style={{ width: "80%" }} />
