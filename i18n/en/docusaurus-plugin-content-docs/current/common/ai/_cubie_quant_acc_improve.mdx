...
Here, we use the uint8 of [MobileNetV2_ImaegNet](./cubie-acuity-usage#quantize-model) from the previous chapter as an example. This model directly uses ACUITY to quantize uint8, and the precision loss is obvious after inference, and the inference result is obviously different from the floating-point model.
