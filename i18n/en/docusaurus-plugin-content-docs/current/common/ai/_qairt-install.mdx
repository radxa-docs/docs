The QAIRT (Qualcomm® AI Runtime) SDK is a software package that integrates Qualcomm® AI software products,
...
Currently, QAIRT SDK supports model porting for frameworks including Pytorch, Tensorflow, TFLite, and ONNX.
...
Running QAIRT requires a Python 3.10 environment, which can be created using [Anaconda](https://www.anaconda.com/download/success)
...
Their detailed documentation can be found at the following locations:
...
Radxa provides a Docker image to facilitate the use of QAIRT SDK on the board. You can also install it manually by following the instructions in this document.

- Install Docker

  :::tip
  Please refer to [**Docker Usage**](../virtual-env/docker_use) for Docker installation
  :::
