...
Includes: user information and password, two-factor authentication, API key, language, device name, device reset, system update.
...
- Scan a QR code with an authenticator app or enter the secret manually
- Require an additional six-digit code during sign-in after 2FA is enabled
- Disable two-factor authentication from the Safety panel

For setup and recovery considerations, see [Two-Factor Authentication (2FA)](/linkr/linkr/two-factor-authentication).
