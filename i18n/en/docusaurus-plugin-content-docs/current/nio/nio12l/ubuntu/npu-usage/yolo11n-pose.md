---
sidebar_position: 20
description: Complete workflow for YOLO11n-Pose model conversion (Host) and deployment (Device) on NIO12L
---
...
```bash
cd examples/yolo11n-pose/convert_model
conda activate yolo-export
...
After conversion, the following files are generated in `examples/yolo11n-pose/model/`:

- `int8/yolo11n-pose_mtk_int8.tflite`
- `fp32/yolo11n-pose_mtk_fp32.tflite`
...
```bash
scp yolo11n-pose_mtk_int8.tflite <user>@<device>:/path/to/nio12l-model-zoo/examples/yolo11n-pose/model/int8/
scp yolo11n-pose_mtk_fp32.tflite <user>@<device>:/path/to/nio12l-model-zoo/examples/yolo11n-pose/model/fp32/
```
...
```bash
cd examples/yolo11n-pose/model/int8
...
Results are saved in `outputs/yolo11n-pose/` (`vis/` for visualization images, `detections/` for JSON).
