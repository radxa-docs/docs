---
sidebar_position: 15
description: Complete workflow for YOLO26n model conversion (Host) and deployment (Device) on NIO12L
---
...
After conversion, the following files are generated in `examples/yolo26n/model/`:

- `int8/yolo26n_mtk_int8.tflite`
- `fp32/yolo26n_mtk_fp32.tflite`
...
```bash
wget -P examples/yolo26n/model/int8 https://github.com/Ronin-1124/nio12l-model-zoo/releases/download/v2026.05.11-dla/yolo26n_int8.dla
wget -P examples/yolo26n/model/fp32 https://github.com/Ronin-1124/nio12l-model-zoo/releases/download/v2026.05.11-dla/yolo26n_fp32.dla
```
...
```bash
scp yolo26n_mtk_int8.tflite <user>@<device>:/path/to/nio12l-model-zoo/examples/yolo26n/model/int8/
scp yolo26n_mtk_fp32.tflite <user>@<device>:/path/to/nio12l-model-zoo/examples/yolo26n/model/fp32/
```
...
```bash
cd examples/yolo26n/model/int8
...
```bash
./build/yolo26n_demo --image assets/images/bus.jpg --image assets/images/zidane.jpg
```
...
Results are saved in `outputs/yolo26n/` (`vis/` for visualization images, `detections/` for JSON).
