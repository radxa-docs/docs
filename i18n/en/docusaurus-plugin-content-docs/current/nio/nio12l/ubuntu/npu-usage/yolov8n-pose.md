---
sidebar_position: 19
description: Complete workflow for YOLOv8n-Pose model conversion (Host) and deployment (Device) on NIO12L
---
...
After conversion, the following files are generated in `yolov8n-pose/`:
...
```bash
scp yolov8n-pose_mtk_int8.tflite <user>@<device>:/path/to/nio12l-model-zoo/models/yolov8n-pose/int8/
scp yolov8n-pose_mtk_fp32.tflite <user>@<device>:/path/to/nio12l-model-zoo/models/yolov8n-pose/fp32/
```
