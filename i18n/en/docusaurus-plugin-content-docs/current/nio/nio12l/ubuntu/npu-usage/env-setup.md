---
sidebar_position: 2
description: Introduces how to install Neuron SDK
---

# Device Environment Setup

Configuring the NPU environment on the device side under Ubuntu is simple. Run the following command in the terminal:
...
The `--edge` channel publishes the latest software packages, which may be updated more frequently than the stable channel. If you want to use it for a stable production environment, you can go to the Snap Store to check each channel's version and official support status.

---

After `mtk-neuropilot` is installed, verify the installation in the terminal:
...
`neuronrt` can run dla models to test model integrity.
